...
# 2.a
Say alpha=0.05
Now all variables are significant. It is not necessarily expected, because the test statistic is a new one too, meaning different p-values.
```{r}
...
model_forward = step(model_empty, scope=formula(lm.full), direction="forward", trace=0)
...
par(mfrow=c(1,2))
plot(x=y_test, y=y_hat_backward, xlim=c(1,3), ylim=c(1.5,2.7))
plot(x=y_test, y=y_hat_forward, xlim=c(1,3), ylim=c(1.5,2.7))
